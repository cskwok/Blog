...
const baseSelectQuery = "SELECT Postid, Title, Content, users.NickName as Author FROM posts LEFT JOIN users on posts.Userid=users.Userid";
...
async function createPost(info) {   
    try {     
        const [rows, fields] = await sql.query("INSERT INTO posts(Userid, Title, Content) VALUES(?, ?, ?)", [info.userid, info.title, info.content]);
    } catch (error) {
